...
It uses n-gram analysis plus two compact word lists for RU↔UK disambiguation —
it's small and fast.
...
occur in real words. A token is treated as a plausible word in language *L* if it has a
vowel and none of its n-grams are blacklisted. For each word it decides whether it looks
like Latin or Cyrillic, "retypes" the keystrokes into the other layout, and keeps the
variant that looks like a real word.

One subtlety: the Russian and Ukrainian layouts differ only on four keys
(`s` `]` `'` `` ` `` → ы/ъ/э/ё vs і/ї/є/ґ), so a wrong-layout token often produces an
n-gram-plausible word in *both* languages (`ghbdsn` → `привыт`/`привіт`). For those ties
Rekey embeds two compact lists of known words containing the layout-specific letters and
picks the real word.
...
(Apache-2.0), extended with Ukrainian support. The Ukrainian data files are generated by
[tools/Rekey.DictGen](tools/Rekey.DictGen) from permissively-licensed corpora:
[ParaCrawl](https://paracrawl.eu/) (CC0 1.0) and
[UA-GEC](https://github.com/grammarly/ua-gec) (CC BY 4.0).
